...
from utils.postprocess import extract_boxes_yolov5, extract_boxes_detectron
...
class RealSenseNode(object):
    def __init__(self, grpc_stub, input_name, output_name, param, FLAGS, dtype, c, h, w):
        ...
        if len(output_name) > 1:  # Models with multiple outputs Boxes, Classes and scores
            ...
            self.output2 = service_pb2.ModelInferRequest().InferRequestedOutputTensor()
            self.output2.name = output_name[2]
            self.output3 = service_pb2.ModelInferRequest().InferRequestedOutputTensor()
            self.output3.name = output_name[3]

            self.request.outputs.extend([self.output0,
                                         self.output1,
                                         self.output2,
                                         self.output3])
        else:
    ...
    def callback(self, msg):
        ...
        cv_image = cv2.resize(cv_image, (self.input.shape[2], self.input.shape[1]))
        ...
        if self.image is not None:
            ...
            self.prediction, self.class_ids, self.scores = extract_boxes_detectron(self.response)

            for object, idx in zip(self.prediction, range(len(self.prediction))):  # predictions array has the order [x1,y1, x2,y2, confidence, confidence, class ID]
            ...
            # cv2.waitKey()
            self.msg_frame = self.br.cv2_to_imgmsg(self.orig_image, encoding="bgr8")
